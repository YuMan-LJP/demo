...
export default {
    ...
    methods: {
        ...
        draw() {
            ...
            for (let key in this.players) {
                ...
                this.players[key].objects.forEach(f => this.ctx.fillRect(f.x * this.scaleSize, f.y * this.scaleSize, this.scaleSize, this.scaleSize));
            }
            //TODO...
        },
    ...
    mounted() {
        console.log("Tank Online mounted");
    ...
    beforeUnmount() {
        console.log('Tank Online beforeUnmount');
        // 组件卸载前断开连接
        if (this.socket) {
            //触发刷新在线人数统计
            this.socket.emit(`back-OtherMessage-${this.otherId}`, {
    ...
    beforeDestroy() {
        console.log("Tank Online beforeDestroy");
        // 组件销毁前断开连接
        if (this.socket) {
            //触发刷新在线人数统计
            this.socket.emit(`back-OtherMessage-${this.otherId}`, {
