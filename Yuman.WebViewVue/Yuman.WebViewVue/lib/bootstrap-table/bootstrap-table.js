	...
	*/
	...
	function () {
	  ...
	  _createClass(BootstrapTable, [{
	    ...
	    value: function initHeader() {
	      ...
	      this.options.columns.forEach(function (columns, i) {
	        ...
	        if (i === 0 && !_this2.options.cardView && _this2.options.detailView && _this2.options.detailViewIcon) {
				try{
					html.push("<th style='vertical-align:middle;' class=\"detail\" rowspan=\"".concat(_this2.options.columns.length, "\">\n          <div class=\"fht-cell\" style='text-align:center;width:32px;'><i title='" + L("ExtendAll") + "' class='glyphicon glyphicon-plus icon-plus bootstrapDetailTableExtall'></i><i title='" + L("CollapseAll")+ "' class='glyphicon glyphicon-minus icon-minus bootstrapDetailTableColall' style='display:none;'></i></div>\n          </th>\n        "));
				}
				catch(e){
					html.push("<th style='vertical-align:middle; class=\"detail\" rowspan=\"".concat(_this2.options.columns.length, "\">\n          <div class=\"fht-cell\" style='text-align:center;width:32px;'><i title='Extend all' class='glyphicon glyphicon-plus icon-plus bootstrapDetailTableExtall'></i><i title='Collapse all' class='glyphicon glyphicon-minus icon-minus bootstrapDetailTableColall' style='display:none;'></i></div>\n          </th>\n        "));
				}
	        }
		  ...
		  var self = this
		  if (!_this2.options.cardView && _this2.options.detailView && _this2.options.detailViewIcon) {
			  	this.$tableBody.find(".bootstrapDetailTableExtall").on('click',function(e){
					e.currentTarget.nextSibling.style.display = 'inline'
					e.currentTarget.style.display = 'none'
					self.expandAllRows()
				});
				this.$tableBody.find('.bootstrapDetailTableColall').on('click',function(e){
	    ...
	    key: "initPagination_Old",
	    value: function initPagination_Old() {
	      var _this6 = this;
	      ...
	      if (!o.onlyInfoPagination) {
           ...
           //  if (this.totalPages <= 1) {
	        ...
	        //} // when data is empty, hide the pagination
              if (o.smartDisplay && !pageList.length) {
                  this.$pagination.find('span.page-list').hide();
              }
           //注释 显示分页信息
		...
		value: function initPagination() {
			...
			var dropPageRender = function (option) {
				var startNumber = 0;
				var endNumber = 0;
				var renderedItems = [];
				if (option.type == "1") {
					startNumber = option.startNumber1;
					endNumber = option.endNumber1;
					renderedItems = dropNumbers1;
				} else if (option.type == "2") {
					startNumber = option.startNumber2;
					endNumber = option.endNumber2;
					renderedItems = dropNumbers2;
				}
				...
				for (var i = 0; i < currDropPage.length; i++) {
					if (renderedItems.indexOf(currDropPage[i]) === -1) {
						if (option.type == "1") {
							dropNumbers1.push(currDropPage[i]);
						} else if (option.type == "2") {
							dropNumbers2.push(currDropPage[i]);
						}
					}
				}
					...
					if (from - 1 === max + 1) {
						...
						if (from - 1 > max) {
							if (from - o.paginationPagesBySide * 2 > o.paginationPagesBySide && o.paginationUseIntermediate) {
								...
								html.push('<li class="page-item page-last-separator">');
								html.push('<span style="padding: 6px 1px">');
								html.push('<a class="page-link" href="javascript:void(0)" data-toggle="dropdown">...</a>')
								var dropPageUl = "";
								if (o.dropPageStyle && o.dropPageStyle !== "") {
									dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer1" style="' + o.dropPageStyle + '">';
								} else {
									if (o.pageSize > 10) {
										dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer1" style="max-height:200px;overflow-y:auto;bottom:100%;top:auto">';
									} else {
										dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer1" style="max-height:200px;overflow-y:auto;">';
									}
								}
								html.push(dropPageUl);

								dropPageNumberOptions.startNumber1 = 1;
								dropPageNumberOptions.endNumber1 = from - 1;
								dropPageNumberOptions.type = "1";

								var currDropPage = dropPageRender(dropPageNumberOptions);

								for (var dropIndex = 0; dropIndex < currDropPage.length; dropIndex++) {
									html.push(pageItem(currDropPage[dropIndex], ' menuitem'));
								}
								html.push('</ul>');
								html.push('</span>');
								html.push('</li>');
					...
					if (to + 1 === min - 1) {
						...
						if (min > to + 1) {
							if (this.totalPages - to > o.paginationPagesBySide * 2 && o.paginationUseIntermediate) {
								...
								var dropPageUl = "";
								if (o.dropPageStyle && o.dropPageStyle !== "") {
									dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer2" style="' + o.dropPageStyle + '">';
								} else {
									if (o.pageSize > 10) {
										dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer2" style="max-height:200px;overflow-y:auto;bottom:100%;top:auto">';
									} else {
										dropPageUl = '<ul class="dropdown-menu dropdown-menu-droppage" id="dropPageContainer2" style="max-height:200px;overflow-y:auto;">';
									}
								}
								html.push(dropPageUl);

								dropPageNumberOptions.startNumber2 = to;
								dropPageNumberOptions.endNumber2 = this.totalPages;
								dropPageNumberOptions.type = "2";
			...
			if (dropNumbers1.length > 0) {
				this.$pagination.find("#dropPageContainer1").on('scroll', function () {
					var $this = $(this);
					dropPageNumberOptions.type = "1"
					dropPageScroll($this);
				});
			...
			if (dropNumbers2.length > 0) {
				this.$pagination.find("#dropPageContainer2").on('scroll', function () {
					var $this = $(this);
					dropPageNumberOptions.type = "2"
					dropPageScroll($this);
				});
			...
			var dropPageScroll = function (m) {
				var $this = m;
				var isBottomReached = ($this.outerHeight() + $this.scrollTop()) >= $this[0].scrollHeight;
				if (isBottomReached) {
					// 滚动条已经到达底部，加载数据
					var currDropPage = dropPageRender(dropPageNumberOptions);
					for (var dropIndex = 0; dropIndex < currDropPage.length; dropIndex++) {
						var newLi = pageItem(currDropPage[dropIndex], ' menuitem');
						$this.append(newLi);
					}
	    ...
	    value: function updatePagination(event) {
		  ...
		  if(this.options.detailView){
			  if(document.getElementsByClassName('bootstrapDetailTableExtall').length){
				  document.getElementsByClassName('bootstrapDetailTableExtall')[0].style.display = 'inline'
				  document.getElementsByClassName('bootstrapDetailTableColall')[0].style.display = 'none'
				}
			}
	    ...
	    value: function fitHeader() {
	     ...
	     // var scrollWidth = fixedBody.scrollWidth > fixedBody.clientWidth && fixedBody.scrollHeight > fixedBody.clientHeight + this.$header.outerHeight() ? Utils.getScrollBarWidth() : 0;
          var   scrollWidth = fixedBody.scrollHeight > fixedBody.clientHeight ? Utils.getScrollBarWidth() : 0;
	      ...
	      $tr.find('> *').each(function (i, el) {
	        ...
	        if (_this14.options.detailView && _this14.options.detailViewIcon && !_this14.options.cardView) {
	          if (i === 0) {
				...
				//// 若升级至jquery3.3.1以上 则使用该代码 注释下面对应的代码  -2022-05-12
				// var _zoomWidth = $thDetail.innerWidth() - $thDetail.find('.fht-cell').width();

				// $thDetail.find('.fht-cell').width($this.innerWidth() - _zoomWidth);

	            var _zoomWidth = $thDetail.width() - $thDetail.find('.fht-cell').innerWidth();
				if(_zoomWidth > 0)
	            	$thDetail.find('.fht-cell').width($this.innerWidth() - ( $this.innerWidth() - $this.width() ) - _zoomWidth);
				else
					$thDetail.find('.fht-cell').width($this.innerWidth() - _zoomWidth);
	          }
			...
			var zoomWidth = $th.width() - $th.find('.fht-cell').innerWidth();
			if(zoomWidth > 0)
	        	$th.find('.fht-cell').width($this.innerWidth() - ( $this.innerWidth() - $this.width() ) - zoomWidth);
			else
	        	$th.find('.fht-cell').width($this.innerWidth() - zoomWidth);
	      });
	    ...
	    value: function getData(params) {
	      ...
	      if (params && !params.includeHiddenRows) {
			...
			//2021-03-26 query contains filtered data
			if (params && params.includeFilter) {
