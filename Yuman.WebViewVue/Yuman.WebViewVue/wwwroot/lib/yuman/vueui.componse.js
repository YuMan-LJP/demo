...
yumanCron = Vue.component('yuman-cron', {
    props: {
        cronexp: {
            type: String,
            default: '',
    ...
    methods: {
        init() {
            $('#cron').val(this.cronexp)
            this.myCronExp = '';
            try {
                $("#cron").cronGen({
                    //direction: 'right',
                    mycustom: 'myCronCustom',//自己改编的，不使用弹出框的形式，改用直接显示到div上
                });
            }
            catch (ex) {
                yuman.message.error(L("TriggerInfo.Cron.FormatError"), L("SystemTips"));
                return;
            }
        },
        showCronNextTime() {
            this.myCronExp = $('#cron').val() + '';
        },
        getResult() {
            return $('#cron').val() + '';
        }
    ...
    template: `
    <div class="form-group">
        <div id="myCronCustom"></div>
        <yuman-button :css="'btn btn-xs btn-success'" :licss="''" :title="L('TriggerInfo.Cron.AntiParsing')" v-on:click="showCronNextTime"></yuman-button>
        <input id="cron" class="form-control" />
        <yuman-cron-shownexttime v-if="myCronExp!=''" :ex="myCronExp"></yuman-cron-shownexttime>
...
* * * * * * 命令
            ...
            <p>除了基本的数字范围外，Cron 表达式还支持一些特殊符号和字符：</p>
            <ul>
...
var yumanCronShowNextTime = Vue.component('yuman-cron-shownexttime', {
    props: {
        ex: {
            type: String,
            required: true
        ...
        getWeekArr(rule) {
            ...
            if (this.dayRule == "" && this.dayRuleSup == "") {
                if (rule.indexOf("-") >= 0) {
                    ...
                    this.dayRule = "lastWeek";
                    this.dayRuleSup = Number(rule.match(/[0-9]{1,2}/g)[0]) - 1;
                    this.dateArr[3] = [31];
